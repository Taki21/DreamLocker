    ...
    useEffect(() => {
        if(active) {
            const loadLocks = async () => {
                const ca = new library.eth.Contract(LockerABI, LockerContract);
                setLockedArray(await ca.methods.getLocks(account, pair).call({from: account}))
            }
            loadLocks();
            getTotalSupply();
        }
            ...
            <main className="flex flex-col">
                ...
                <div className="px-8">
                    <div className='flex'>
                        ...
                        <h1 className='bg-gradient-to-r from-[#ffffff] to-[#ffddff] bg-clip-text text-transparent p-8 text-2xl font-bold self-center'>My Locked Liquidity</h1>
                    </div>
                    
                    <div className="flex flex-col items-center justify-center mt-2">
                        {active ? (
                            lockedArray.map((i, j) => (
                            <div className="flex flex-col w-full bg-gradient-to-br p-8 from-[#653877] to-[#342138] rounded-xl mt-4 text-white text-xs lg:text-base">
                                <div className="flex items-center">
                                    ...
                                    <h1 className="font-bold text-xl ml-1">UNI-V2 Pair</h1>
                                </div>
                                ...
                                <button className='p-4 mt-4 bg-[#6b4a77] hover:bg-[#583c62] rounded-2xl w-full text-white' onClick={() => unlock(i[0], j)}>Withdraw</button>
                            </div>  
                            ))
                        ) : <Wallet/>}
                    </div>
